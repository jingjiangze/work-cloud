...
def _default_account_config(phone: str, password: str) -> dict:
    ...
    2026-09-25 用户口径：所有账号关闭日报、周报/月报全开（AI 配置
    继承 me）；默认定位威海智慧谷（真实实习岗位围栏内，2026-09-25
    实测谷庄村定位会被岗位考勤判无效——App 显示"今日未打卡"）。
    """
    return {"config": {
        "user": {"phone": phone, "password": password},
        "clockIn": {
            "enabled": True, "mode": "daily",
            "location": {"address": "山东省 · 威海市 · 环翠区 · 在智慧谷附近",
                         "latitude": "37.428491", "longitude": "122.184165",
                         "province": "山东省", "city": "威海市", "area": "环翠区"},
            "imageCount": 0,
